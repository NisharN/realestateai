  ...
  const field = (name: string, label: string, props: React.InputHTMLAttributes<HTMLInputElement> = {}) => (
    <label className="grid gap-1 text-xs font-medium text-muted-foreground">
      ...
      <input
        ...
        className="rounded-lg border px-3 py-2 text-sm font-normal text-foreground"
        {...props}
      />
    </label>
  );
  ...
  return (
    <main className="px-5 py-8 lg:px-10">
      <div className="mx-auto max-w-6xl">
        <Link href="/broker" className="text-sm text-muted-foreground hover:text-brand">
          ← Broker Today
        </Link>
          ...
          <>
            <header className="mt-3 flex flex-wrap items-start justify-between gap-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="rounded-full bg-card px-3 py-1 text-sm ring-1 ring-border">{STAGE_LABELS[lead.stage] ?? lead.stage}</span>
              </div>
            </header>

            {notice && <p role="status" className="mt-4 rounded-xl bg-brand/5 p-3 text-sm text-brand">{notice}</p>}

            <div className="mt-6 grid gap-6 lg:grid-cols-[1.2fr_1fr]">
              <div className="grid gap-6">
                <section className="ui-card p-5">
                  <h2 className="ui-kicker">Brief</h2>
                  {brief ? (
                    <div className="mt-3 grid gap-2 text-sm">
                      <p className="text-base font-medium">{brief.headline}</p>
                      <p className="text-foreground/80">Next step: {brief.next_step}</p>
                      ...
                      {Array.isArray(brief.objections) && brief.objections.length > 0 && (
                        <p className="text-muted-foreground">Objections: {brief.objections.join("; ")}</p>
                      )}
                    </div>
                  ) : (
                    <p className="mt-3 text-sm text-muted-foreground">No handoff brief yet — this lead has not been handed off.</p>
                  )}
                ...
                {data.handoff && (
                  <section className="ui-card p-5">
                    <h2 className="ui-kicker">Handoff</h2>
                    <div className="mt-3 flex flex-wrap items-center gap-3 text-sm">
                      <span className="capitalize">{data.handoff.status}</span>
                      <span className="text-muted-foreground">to {data.handoff.broker_name ?? data.handoff.broker_id ?? "unassigned"}</span>
                      {data.handoff.slot_text && <span className="text-muted-foreground">wants {data.handoff.slot_text}</span>}
                      <span className="text-muted-foreground/70">{relativeTime(data.handoff.created_at)}</span>
                      {data.handoff.status === "pending" && (
                        <span className="ml-auto flex gap-2">
                          <button onClick={() => act("accept")} className="rounded-lg bg-brand px-3 py-1.5 text-xs font-semibold text-white">
                            Accept
                          </button>
                    ...
                    {Array.isArray(data.handoff.routing_reasons) && data.handoff.routing_reasons.length > 0 && (
                      <p className="mt-2 text-xs text-muted-foreground">Routed by: {data.handoff.routing_reasons.join(", ")}</p>
                    )}
                  </section>
                )}

                <section className="ui-card p-5">
                  <h2 className="ui-kicker">Timeline</h2>
                    ...
                    <ol className="mt-3 grid gap-2 text-sm">
                      {data.timeline.map((item, i) => (
                        <li key={i} className="flex gap-3">
                          <span className="w-16 shrink-0 text-xs text-muted-foreground/70">{relativeTime(item.at)}</span>
                          <span className={item.kind === "message" ? "text-slate-800" : "text-muted-foreground"}>{describe(item)}</span>
                        </li>
                      ))}
                    </ol>
                  )}
              ...
              <div className="grid gap-6">
                <form onSubmit={save} className="ui-card p-5">
                  <h2 className="ui-kicker">Edit requirements</h2>
                  <p className="mt-1 text-xs text-muted-foreground">Current: {formatBudget(lead)}. Edits are logged and mark fields as broker-confirmed.</p>
                  ...
                  <button disabled={saving} className="mt-4 rounded-xl bg-brand px-5 py-2.5 text-sm font-semibold text-white disabled:opacity-50">
                    {saving ? "Saving…" : "Save changes"}
                  </button>
                </form>

                <section className="ui-card p-5 text-sm">
                  <h2 className="ui-kicker">Sources & follow-ups</h2>
                  {data.sources.length === 0 ? (
                    <p className="mt-3 text-muted-foreground">No ingestion sources — created from conversation.</p>
                  ) : (
                  ...
                  <h3 className="mt-4 text-xs font-semibold uppercase text-muted-foreground">Viewings</h3>
                  {data.viewings.length === 0 ? (
                    <p className="mt-1 text-muted-foreground">No viewings yet.</p>
                  ) : (
                    <ul className="mt-1 grid gap-2 text-foreground/80">
                      {data.viewings.map((v) => (
                        <li key={v.id} className="flex flex-wrap items-center gap-2">
                          <span className="rounded bg-muted px-1.5 py-0.5 text-xs font-medium">{VIEWING_LABELS[v.status]}</span>
                          <span>{formatWhen(v.starts_at)}</span>
                          {v.property_id && <span className="text-xs text-muted-foreground">property {v.property_id}</span>}
                          <span className="text-xs text-muted-foreground/70">via {v.source}</span>
                          {VIEWING_NEXT[v.status].map((next) => (
                            <button
                              key={next}
                              type="button"
                              onClick={() => void setViewingStatus(v.id, next)}
                              className="rounded border border-border px-2 py-0.5 text-xs hover:bg-muted/50"
                            >
                              {VIEWING_LABELS[next]}
                            </button>
                          ))}
                  ...
                  <form onSubmit={bookViewing} className="mt-2 grid gap-2 rounded-lg border border-border p-3">
                    <p className="text-xs font-medium text-muted-foreground">Book a viewing</p>
                    <input
                      type="datetime-local"
                      required
                  ...
                  <h3 className="mt-4 text-xs font-semibold uppercase text-muted-foreground">Follow-ups</h3>
                  {data.followups.length === 0 ? (
                    <p className="mt-1 text-muted-foreground">None scheduled.</p>
                  ) : (
                    <ul className="mt-1 grid gap-1 text-foreground/80">
