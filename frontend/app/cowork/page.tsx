...
type Tab = (typeof TABS)[number][0] | (typeof ADVANCED)[number][0];
...
function Cowork() {
  ...
  const tab: Tab = ALL.some(([k]) => k === requested) ? (requested as Tab) : "connections";
  ...
  const setTab = (next: Tab, subTab?: IngestionTab) => {
    setMoreOpen(false);
    router.replace(next === "connections" ? "/cowork" : `/cowork?tab=${next}${subTab ? `&sub=${subTab}` : ""}`);
  };
