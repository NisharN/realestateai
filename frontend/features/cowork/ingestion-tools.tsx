...
export type IngestionTab = "csv" | "review" | "health" | "ops";

export const INGESTION_TABS: readonly [IngestionTab, string][] = [
  ["csv", "CSV upload"],
  ["review", "Review queue"],
  ["health", "Data health"],
  ["ops", "Ops"],
...
export function Notices({ message, error }: { message: string | null; error: string | null }) {
  return (
    <>
...
/** CSV upload, review queue, data health and Ops — the data tooling that used to live under Admin → Ingestion. */
export function IngestionTools({ initialTab = "csv" }: { initialTab?: IngestionTab }) {
  const [tab, setTab] = useState<IngestionTab>(initialTab);
  const { say, message, error } = useSay();

  return (
    <div>
      <div role="tablist" aria-label="Data tools" className="flex gap-1 rounded-xl border border-border bg-card p-1 text-sm font-medium shadow-card">
        {INGESTION_TABS.map(([key, label]) => (
          <button
            key={key}
            role="tab"
...
export type Say = (result: ApiResponse<unknown>, ok: string) => boolean;

export function ConnectorsTab({ say }: { say: Say }) {
